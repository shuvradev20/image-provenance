...
export const CONTRACT_ADDRESS = "0xc9EBa5fB6D3c74B47568a1917dd31303E158a059";
...
export const CONTRACT_ABI = [
    ...
    "function registerImage(bytes32 _imageHash, bytes4 _watermarkID, string calldata _metadataCID, bytes calldata _signature) external",
    ...
    // --- View/Read Functions ---
    "function verify(bytes32 _newHash, bytes4 _extractedWatermarkID) external view returns (string status, address originalOwner)",
    "function getUserImages(address _user) external view returns (bytes32[])",
    "function getImageDetails(bytes32 _hash) external view returns (tuple(bytes32 imageHash, bytes4 watermarkID, string metadataCID, address currentOwner, uint64 timestamp, bool isBurned))",
    
    // --- Public State Variables (Getters) ---
    "function totalImages() external view returns (uint256)",
    "function images(bytes32) external view returns (bytes32 imageHash, bytes4 watermarkID, string metadataCID, address currentOwner, uint64 timestamp, bool isBurned)",
    "function imageExists(bytes32) external view returns (bool)",
    "function watermarkToOriginal(bytes4) external view returns (bytes32)",
