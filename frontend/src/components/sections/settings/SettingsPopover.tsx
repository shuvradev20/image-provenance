...
export function SettingsPopover({ children }: { children: React.ReactNode }) {
  ...
  const menuItemStyle = "py-2 px-3 cursor-pointer rounded-md transition-colors hover:bg-zinc-200/80 dark:hover:bg-zinc-800/80 text-foreground text-xs font-medium"
            ...
            >
              ...
              <div className="flex flex-col gap-1">
                ...
                <button
                  ...
                  className={`${menuItemStyle} flex items-center w-full text-left`} onClick={() => setMobileMenuOpen(false)}>
  ...
  return (
    <DropdownMenu>
      ...
      >
        <DropdownMenuSub>
          <DropdownMenuSubTrigger title="Change Appearance Theme" className={menuItemStyle}>
            <Palette className="mr-2 h-4 w-4 text-muted-foreground" />
            <span>Theme</span>
          </DropdownMenuSubTrigger>
          
          <DropdownMenuPortal>
            {/* Desktop Theme SubMenu Content */}
            <DropdownMenuSubContent className="p-1 w-40 bg-popover text-popover-foreground border border-border rounded-xl shadow-xl z-50">
              <DropdownMenuItem title="Set Light Theme" onClick={() => setTheme("light")} className={`${getActiveStyle("light")} flex items-center justify-between`}>
                <div className="flex items-center">
                  <Sun className="mr-2 h-3.5 w-3.5 text-muted-foreground" />
                  <span>Light</span>
                </div>
              ...
              <DropdownMenuItem title="Set Dark Theme" onClick={() => setTheme("dark")} className={`${getActiveStyle("dark")} flex items-center justify-between`}>
                <div className="flex items-center">
                  <Moon className="mr-2 h-3.5 w-3.5 text-muted-foreground" />
                  <span>Dark</span>
                </div>
              ...
              <DropdownMenuItem title="Set System Default Theme" onClick={() => setTheme("system")} className={`${getActiveStyle("system")} flex items-center justify-between`}>
                <div className="flex items-center">
                  <Monitor className="mr-2 h-3.5 w-3.5 text-muted-foreground" />
                  <span>System</span>
                </div>
        ...
        <DropdownMenuItem title="Manage Subscription Details" className={menuItemStyle}>
          <CreditCard className="mr-2 h-4 w-4 text-muted-foreground" />
          <span>Manage Subscription</span>
        </DropdownMenuItem>

        <DropdownMenuItem title="Get Help & Documentation" className={menuItemStyle}>
          <HelpCircle className="mr-2 h-4 w-4 text-muted-foreground" />
          <span>Help</span>
        </DropdownMenuItem>

        <DropdownMenuItem title="Provide Feedback" className={menuItemStyle}>
          <MessageSquare className="mr-2 h-4 w-4 text-muted-foreground" />
